...
DOMAIN_NAME = os.getenv('DOMAIN_NAME', "http://127.0.0.1:5032")

#DOMAIN_NAME = os.getenv('DOMAIN_NAME', f"http://{request.host}")# os.environ['DOMAIN_NAME']
# OTP Configuration
...
def login_page():
    if request.method == 'POST':
        ...
        if success:
            ...
            if role == 'admin':
                return redirect(f'{DOMAIN_NAME}/admin')
            else:
