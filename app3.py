from quart import Quart, render_template, request, redirect, url_for, send_file, flash, send_from_directory, jsonify
import os
import json
import aiohttp
import asyncio
...
for folder in ["json", "reports", os.path.join("static", "uploads"), "audio", "logos"]:
    os.makedirs(os.path.join(app.root_path, folder), exist_ok=True)

# Path to store submissions metadata
SUBMISSIONS_JSON = os.path.join(app.root_path, "json", "submissions.json")
...
async def process_video(user_name: str, video_path: str, presentation_mode: str, session, output_dir: str) -> dict:
    try:
        ...
        # Analyze video and audio concurrently
        ...
        # Update output JSON with user name and LLM results
        ...
        submission = {
            ...
            'score': output.get('overall_score', 0.0),
            'status': 'analyzed',
...
async def index():
    if request.method == "POST":
        ...
        presentation_mode = form.get("presentation_mode", "speech")
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                result = await process_video(user_name, video_path, presentation_mode, session, uploads_dir)
